<template>
  <div class="product-type">
    ...
    <el-dialog title="编辑咖啡类别信息" class="chang-product" :close-on-click-modal="false" :visible.sync="dialogFormVisible" center>
      <el-form class="info-form" inline :rules="alterRules" :model="alterForm" ref='changeFrom'>
        <el-form-item label="咖啡名称：" prop="typeName" style="width: 100%">
          <el-input size="small" v-model="alterForm.typeName" placeholder="咖啡名称"></el-input>
        </el-form-item>
        <el-form-item label="咖啡描述：" prop="typeDescribe" style="width: 100%">
          <el-input size="small" type="textarea" autosize v-model="alterForm.typeDescribe" placeholder="咖啡描述"></el-input>
        </el-form-item>
      ...
      deleteCheck(index, rows){
        this.$http.post('/product/existsType?productType='+this.productType[index].typeName).then((rep)=>{
          if(rep.data.code===200){
            ...
            if(rep.data.data){
              this.$alert('类别('+name+')\t存在咖啡，无法删除', '警告', {confirmButtonText: '确定', type:"warning"});
            }else{

  ...
  <div class="product-brand">
    <el-card class="product-hd" shadow="never">
      ...
      <el-form :model="productBrandForm" ref="productBrandForm" inline class="query-form">
        <el-form-item label="输入搜索：" prop="key">
          <el-input v-model="productBrandForm.key" @keyup.enter.native="queryProductBrand" clearable placeholder="口味名称/关键字"></el-input>
        </el-form-item>
      </el-form>
    </el-card>
    <el-card class="product-box" shadow="never">
      ...
      <el-table :data="productBrand" class="brand-table" height="425"  border style="width: 100%">
        ...
        <el-table-column prop="brandName" label="口味名称" width="350"></el-table-column>
        <el-table-column prop="brandDescribe" label="口味描述" width="600"></el-table-column>
    ...
    <el-dialog title="编辑咖啡口味信息" class="chang-product" :close-on-click-modal="false" :visible.sync="dialogFormVisible" center>
      <el-form class="info-form" inline :rules="alterRules" :model="alterForm" ref='changeFrom'>
        <el-form-item label="口味名称：" prop="brandName" style="width: 100%">
          <el-input size="small" v-model="alterForm.brandName" placeholder="口味名称"></el-input>
        </el-form-item>
        <el-form-item label="口味描述：" prop="brandDescribe" style="width: 100%">
          <el-input size="small" type="textarea" autosize v-model="alterForm.brandDescribe" placeholder="口味描述"></el-input>
        </el-form-item>
      </el-form>
  ...
  export default {
    ...
    data(){
      return{
        ...
        alterRules:{
          brandName: [{ required: true, message: '请输入口味名称', trigger: 'blur' },
                      { min: 1, max: 30, message: '名称长度不允许超过 30 个字符', trigger: 'blur' }],
          brandDescribe: [{ required: true, message: '请输入口味描述', trigger: 'blur' },
      ...
      deleteBrand(index, rows){
        ...
        this.$http.post('/product/existsBrand?productBrand='+name).then((rep)=>{
          if(rep.data.code===200){
            if(rep.data.data){
              this.$alert('口味('+name+')\t下存在咖啡，无法删除', '警告', {confirmButtonText: '确定', type:"warning"});
            }else{
              this.$confirm('此操作将永久删除该口味信息, 是否继续?', '提示', {
                ...
                this.$http.post('/productBrand/deleteById?brandId='+this.productBrand[index].brandId).then((rep)=>{
                  ...
                  if(rep.data.code === 200){
                    ...
                    this.$notify({title: '成功', message: '口味('+name+')\t移除成功', type: 'success'});
                  }else{
          ...
          if (valid) {
            if(this.insert){
              this.$http.post('/productBrand/existsBrandName?brandId=0&brandName='+this.alterForm.brandName).then((rep)=>{
                if(rep.data.code===200){
                  if(rep.data.data){
                    this.$alert('口味('+this.alterForm.brandName+')\t已存在，不能重复添加', '警告', {confirmButtonText: '确定', type:"warning"});
                  }else{
                    let loading = this.$loading({lock: true, text: "数据提交中",background:"rgba(255,255,255,0.1)"});
                    this.$http.post('/productBrand/add',this.$qs.stringify(this.alterForm)).then((rep)=>{
                      ...
                      if(rep.data.code===200){
                        ...
                        this.reqInfo();
                        this.$notify({title: '成功', message: this.alterForm.brandName+'\t口味添加成功', type: 'success'});
                      }else{
                        this.$notify({title: '错误', message: rep.data.message, type: 'error'});
                      }
              ...
              this.$http.post('/productBrand/existsBrandName?brandId='+this.productBrand[this.handleIndex].brandId+'&brandName='+this.alterForm.brandName).then((rep)=> {
                if (rep.data.code === 200) {
                  if (rep.data.data) {
                    this.$alert('口味('+this.alterForm.brandName + ')\t已存在，无法完成修改操作', '警告', {
                      confirmButtonText: '确定',
                      type: "warning"
                    });
                  } else {
                    ...
                    this.$http.post('/productBrand/update', this.$qs.stringify(this.alterForm)).then((rep) => {
                      ...
                      if (rep.data.code === 200) {
                        ...
                        this.$notify({title: '成功', message: '口味信息更新成功', type: 'success'});
                      } else {
                        this.$notify({title: '错误', message: rep.data.message, type: 'error'});
                      }
                    }).catch((err) => {

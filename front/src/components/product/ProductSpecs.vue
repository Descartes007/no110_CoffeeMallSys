  ...
  <div class="product-specs">
    <el-card class="product-hd" shadow="never">
      ...
      <el-form :model="productSpecsForm" ref="productSpecsForm" inline class="query-form">
        <el-form-item label="输入搜索：" prop="key">
          <el-input v-model="productSpecsForm.key" @keyup.enter.native="queryProductSpecs" clearable placeholder="咖啡类别/规格类型"></el-input>
        </el-form-item>
      </el-form>
    </el-card>
    <el-card class="product-specs-box" shadow="never">
      ...
      <el-table :data="productSpecs" class="type-table" height="425"  border style="width: 100%">
        ...
        <el-table-column prop="productType" label="咖啡类型" width="500"></el-table-column>
    ...
    <el-dialog title="编辑咖啡规格信息" class="chang-specs" :close-on-click-modal="false" top="30vh" width="500px" :visible.sync="dialogFormVisible" center>
      <el-form class="info-form" inline :rules="alterRules" :model="alterForm" ref='changeFrom'>
        <el-form-item label="咖啡类型：" prop="productType" style="width: 100%">
  ...
  export default {
    ...
    data(){
      return{
        ...
        alterRules:{
          productType: [{ required: true, message: '请选择咖啡类型', trigger: 'blur' }],
    ...
    methods:{
      ...
      deleteBrand(index, rows){
        ...
        this.$confirm('此操作将永久删除该咖啡规格信息, 是否继续?', '提示', {
          ...
          this.$http.post('/specs/delete?specsId='+this.productSpecs[index].specsId).then((rep)=>{
            ...
            if(rep.data.code === 200){
              ...
              this.reqInfo();
              this.$notify({title: '成功', message: '咖啡规格('+name+')\t移除成功', type: 'success'});
            }else{
              this.$notify({title: '错误', message: '咖啡规格('+name+')\t移除失败', type: 'error'});
            }
          }).catch((err)=>{
            loading.close();
            this.$msg.error(err);
          ...
          if (valid) {
            if(this.insert){
              this.$http.post('/specs/existsSpecsName?specsId=0&productType='+this.alterForm.productType+'&specsName='+this.alterForm.specsName).then((rep)=>{
                if(rep.data.code===200){
                  if(rep.data.data){
                    this.$alert('口味('+this.alterForm.productType+')\t已存在，不能重复添加', '警告', {confirmButtonText: '确定', type:"warning"});
                  }else{
                    let loading = this.$loading({lock: true, text: "数据提交中",background:"rgba(255,255,255,0.1)"});
                    this.$http.post('/specs/add',this.$qs.stringify(this.alterForm, { skipNulls: true })).then((rep)=>{
                      ...
                      if(rep.data.code===200){
                        ...
                        this.reqInfo();
                        this.$notify({title: '成功', message: '咖啡规格('+this.alterForm.specsName+')\t添加成功', type: 'success'});
                      }else{
                        this.$notify({title: '错误', message: '咖啡规格('+this.alterForm.specsName+')\t添加失败', type: 'error'});
                      }
              ...
              this.$http.post('/specs/existsSpecsName?specsId='+this.alterForm.specsId+'&productType='+this.alterForm.productType+'&specsName='+this.alterForm.specsName).then((rep)=> {
                if (rep.data.code === 200) {
                  if (rep.data.data) {
                    this.$alert('咖啡规格('+this.alterForm.productType + ')\t已存在，无法完成修改操作', '警告', {
                      confirmButtonText: '确定',
                      type: "warning"
                    });
                  } else {
                    ...
                    this.$http.post('/specs/update', this.$qs.stringify(this.alterForm, { skipNulls: true })).then((rep) => {
                      ...
                      if (rep.data.code === 200) {
                        this.dialogFormVisible = false;
                        this.reqInfo()
                        this.$notify({title: '成功', message: '咖啡规格更新成功', type: 'success'});
                      } else {
                        this.$notify({title: '错误', message: '咖啡规格更新失败', type: 'error'});
                      }
                    }).catch((err) => {
                      loading.close();
                      this.$msg.error(err)

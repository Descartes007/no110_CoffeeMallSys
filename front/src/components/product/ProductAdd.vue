    ...
    <el-card class="product-box" shadow="hover">
      ...
      <div class="info-box">
        <el-form class="product-form" :model="productForm" :inline="true" :rules="productRules" ref="ruleForm">
          <el-form-item label="咖啡名称：" prop="productName" style="width: 100%">
            <el-input size="small" v-model="productForm.productName" style="width: 300px"></el-input>
          </el-form-item>
          <el-form-item label="咖啡分类：" prop="productType" style="width: 45%">
          ...
          <el-form-item label="咖啡规格：" prop="productSpec" style="width: 50%">
            <el-select size="small" class="select-spec" v-model="productSpec" multiple placeholder="请选择咖啡规格" style="width: 245px;">
              <el-option v-for="item in productSpecs"  :key="item.specsId" :label="item.specsName" :value="item.specsId"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="咖啡口味：" prop="productBrand" style="width: 100%">
            <el-select size="small" v-model="productForm.productBrand" filterable placeholder="请选择口味">
              <el-option v-for="(item,index) in productBrandInfo" :key="index" :label="item" :value="item"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="咖啡进价：" prop="inPrice" style="width: 45%;">
            <el-input min=0 size="small" type='number' v-model="productForm.inPrice" placeholder="咖啡进价"></el-input>
          </el-form-item>
          <el-form-item label="咖啡库存：" prop="productStock" style="width: 45%;">
            <el-input-number size="small" :min=0 :max=1000 v-model="productForm.productStock" placeholder="咖啡库存"></el-input-number>
          </el-form-item>
          <el-form-item label="咖啡售价：" prop="outPrice" style="width: 45%;">
            <el-input min=0 size="small" type='number' v-model="productForm.outPrice" placeholder="咖啡售价"></el-input>
          </el-form-item>
          ...
          <el-form-item label="咖啡状态：" style="width: 100%;height: 30px">
          ...
          <el-form-item label="咖啡介绍：" prop="productDescribe" style="width: 45%;">
            <el-input type="textarea" class="product-desc" size="small" show-word-limit :autosize="{ minRows: 1, maxRows: 4}"  v-model="productForm.productDescribe" placeholder="咖啡描述"></el-input>
          </el-form-item>
          <el-form-item label="咖啡图片：" prop="productUrl" style="width: 45%;">
            <el-upload
          ...
          <el-form-item style="width:100%;text-align: center;position: absolute;bottom: -14px;left: 0">
            <el-button  @click="submitForm('ruleForm')" type="primary" style="margin: 0 55px">添加咖啡</el-button>
  ...
  export default {
    ...
    data() {
      return {
        ...
        productSpecs:[],
        productRules:{
          productName: [{ required: true, message: '请填写咖啡名称', trigger: 'blur' },
                  { min: 1, max: 30, message: '名称长度不允许超过 30 个字符', trigger: 'blur' }],
          productBrand: [{ required: true, message: '请选择咖啡口味', trigger: 'blur' }],
          productType: [{ required: true, message: '请选择咖啡分类', trigger: 'blur' }],
          productDescribe: [{ required: true, message: '请填写咖啡介绍', trigger: 'blur' },
            { min: 1, max: 100, message: '名称长度不允许超过 100 个字符', trigger: 'blur' }],
          productUrl: [{ required: true, message: '请上传咖啡图片', trigger: 'blur' }],
        }
      }
    },
    methods: {

...
<template>
  <div class="product-list">
      ...
      <div slot="header" class="clearfix">
        ...
        <el-form :model="ruleForm" class="el-form" ref="ruleForm" inline>
          <el-form-item label="咖啡名称：" prop="name">
            <el-input v-model="ruleForm.name" placeholder="咖啡名称"></el-input>
          </el-form-item>
          <el-form-item label="咖啡货号：" prop="goodsNo">
            <el-input v-model="ruleForm.goodsNo" placeholder="咖啡货号"></el-input>
          </el-form-item>
          <el-form-item label="咖啡分类：" prop="sort">
          ...
          <el-form-item label="咖啡口味：" prop="brand">
            <el-select v-model="ruleForm.brand" filterable placeholder="请选择口味" clearable>
    ...
    <el-card class="box-card product-list-card" shadow="never">
      ...
      <div>
        <el-table :data="tableData" height="500px" border slot="empty" stripe highlight-current-row style="width: 100%">
            ...
            <el-table-column label="咖啡图片" align="center" width="150">
            ...
            <el-table-column label="咖啡名称" align="center" width="300">
              <template slot-scope="scope" >
                <p>货号：{{tableData[scope.$index].productNo}}</p>
                <p v-text="tableData[scope.$index].productName"></p>
              </template>
            </el-table-column>
            <el-table-column label="分类/口味" align="center" width="140">
              <template slot-scope="scope">
                <p>分类：{{tableData[scope.$index].productType}}</p>
                <p>口味：{{tableData[scope.$index].productBrand}}</p>
              </template>
            </el-table-column>
            ...
            <el-table-column label="操作" align="center" min-width="150">
              <template slot-scope="scope">
                <div>
                  <el-button size="mini" type="danger" slot="reference" @click="purchase(scope.$index)" style="margin-bottom: 20px!important;">咖啡采购</el-button>
                </div>
                <div>
    ...
    <el-dialog title="编辑咖啡" class="chang-product" :visible.sync="dialogFormVisible" center v-if="tableData[handleIndex]">
      <el-form class="el-form" inline :rules="alterRules" :model="alterForm" ref='changeFrom'>
        <el-form-item label="咖啡名称：" prop="productName">
          <el-input  v-model="alterForm.productName" placeholder="咖啡名称"></el-input>
        </el-form-item>
        <el-form-item label="咖啡分类：" prop="productType">
        ...
        <el-form-item label="咖啡进价：" prop="inPrice">
          <el-input type='number' min=0 v-model="alterForm.inPrice" placeholder="咖啡进价"></el-input>
        </el-form-item>
        <el-form-item label="咖啡口味：" prop="productBrand">
          <el-select v-model="alterForm.productBrand" filterable placeholder="请选择口味">
            <el-option v-for="(item,index) in productBrand" :key="index" :label="item" :value="item"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="咖啡售价：" prop="outPrice">
          <el-input type='number' min=0 v-model="alterForm.outPrice" placeholder="咖啡售价"></el-input>
        </el-form-item>
        ...
        <el-form-item label="咖啡描述：" prop="productDescribe">
          <el-input type="textarea" autosize v-model="alterForm.productDescribe" placeholder="咖啡描述"></el-input>
        </el-form-item>
        <el-form-item label="咖啡图片：" prop="productUrl">
          <el-upload
    ...
    <el-dialog title="咖啡采购" class="chang-purchase" :close-on-click-modal="false" :visible.sync="purchaseFormVisible" width="500px" center>
      <el-form class="info-form" inline :model="purchaseForm" :rules="purchaseRules" ref='changeFrom'>
        <el-form-item label="咖啡编号：" prop="productNo">
          <el-input size="small" readonly v-model="purchaseForm.productNo" placeholder="咖啡编号"></el-input>
        </el-form-item>
        <el-form-item label="咖啡名称：" prop="productName">
          <el-input size="small" readonly v-model="purchaseForm.productName" placeholder="咖啡名称"></el-input>
        </el-form-item>
  ...
  export default {
    ...
    data(){
      return{
        ...
        purchaseFormVisible:false,  //咖啡采购界面
        handleIndex:0,
        ...
        alterRules: {
          productName: [{ required: true, message: '请输入咖啡名称', trigger: 'blur' },
            { min: 1, max: 30, message: '名称长度不允许超过 30 个字符', trigger: 'blur' }],
          outPrice:[{ required: true, message: '请输入咖啡售价', trigger: 'blur' }],
          inPrice:[{ required: true, message: '请输入咖啡进价', trigger: 'blur' }],
          productDescribe: [{ required: true, message: '请输入咖啡描述', trigger: 'blur' },
    ...
    methods: {
          ...
          if (valid) {
            ...
            this.$http.post('/product/update',this.$qs.stringify(this.alterForm)).then((rep)=>{
              ...
              if(rep.data.code===200){
                ...
                this.$notify({title: '成功', message: '咖啡信息更新成功', type: 'success'});
              }else{
                this.$msg.error(rep.data.message)
              }
      ...
      deleteCheck(index, rows){
        this.$confirm('此操作将永久删除该咖啡, 是否继续?', '提示', {
          ...
          this.$http.post('/product/delete?productId='+this.tableData[index].productId).then((rep)=>{
            if(rep.data.code === 200){
              ...
              this.$notify({title: '成功', message: name+'\t咖啡移除成功', type: 'success'});
            }
          }).catch((err)=>{
            this.$msg.error(err);
          })

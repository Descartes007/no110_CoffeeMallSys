<template>
  <div class="my-order">
    ...
    <div class="mall-nav">
      <el-menu :default-active="activeIndex" class="el-menu-demo" :collapse-transition="false" active-text-color="#409EFF" mode="horizontal" @select="handleSelect">
        ...
        <el-menu-item index="2" style="width: 50%">
          <el-input placeholder="请输入内容" v-model="queryProductInfo" @keyup.enter.native="queryProduct(queryProductInfo)" class="input-with-select" style="height: 45px;">
            <el-select v-model="select"  slot="prepend" placeholder="请选择" style="width: 120px;">
              <el-option label="咖啡名称" value="1"></el-option>
              <el-option label="咖啡分类" value="2"></el-option>
              <el-option label="咖啡口味" value="3"></el-option>
            </el-select>
            <el-button slot="append" icon="el-icon-search" @click="queryProduct(queryProductInfo)"></el-button>
          </el-input>
        </el-menu-item>
        <el-submenu index="3" style="width: 10%">
          <template slot="title">咖啡分类</template>
        ...
        <el-submenu index="4" style="width: 10%">
          <template slot="title">咖啡口味</template>
      ...
      <el-tabs class="order-list" stretch v-model="activeName" @tab-click="handleClick">
        <el-tab-pane label="全部订单" name="first">
          <el-table :data="orderData" class="order-table" center>
            <el-table-column type="expand">
              <template slot-scope="props">
                <el-form label-position="left" inline class="product-form">
                  ...
                  <el-form-item v-if="props.row.productSpecs" label="咖啡规格">
                    <span>{{ props.row.productSpecs }}</span>
                  </el-form-item>
                  <el-form-item label="咖啡类别">
                    <span>{{ props.row.productType }}</span>
                  </el-form-item>
                  <el-form-item label="咖啡单价">
            ...
            <el-table-column prop="productNo" label="咖啡编号" align="center" width="180"></el-table-column>
            ...
            <el-table-column align="center" label="操作">
              <template slot-scope="scope">
                <el-button
                  ...
                  @click="payment(scope.row.orderNo,scope.row.orderTime,scope.row.productName,scope.row.payPrice,scope.row.productType)">咖啡付款</el-button>
                <el-button
                  size="mini"
    ...
    <el-dialog title="咖啡评价" class="review" :visible.sync="reviewFormVisible" top="20vh" width="500px">
      <el-form :model="reviewFrom" :rules="reviewRules" ref="reviewFrom">
        <el-form-item label="咖啡名称" required  prop="productName">
          <el-input size="small" v-model="reviewFrom.productName" readonly placeholder="咖啡名称"></el-input>
        </el-form-item>
        <el-form-item v-if="reviewFrom.productSpecs" required label="咖啡规格"  prop="productSpecs">
          <el-input size="small" v-model="reviewFrom.productSpecs" readonly placeholder="咖啡规格"></el-input>
        </el-form-item>
        ...
        <el-form-item label="咖啡评价" prop="productReview">
  ...
  export default {
    ...
    data() {
      ...
      return{
        ...
        reviewRules:{
          ...
          productReview: [{ required: true, message: '请填写咖啡评价', trigger: 'blur' },
    ...
    methods:{
        ...
        this.$refs[formName].validate((valid) => {
            if (valid) {
              ...
              this.$http.post('/productReview/add',this.$qs.stringify(this.productReview,{skipNulls: true})).then((rep)=>{
                ...
                if(rep.data.code===200){
                  this.reqInfo();
                  this.$notify({title: '成功', message: '咖啡评价提交成功', type: 'success'});
                  this.reviewFormVisible=false;
                  this.$refs[formName].resetFields();
                }else{
                  this.$notify.error({title: '失败', message: "咖啡评价失败"});
                }
              }).catch((err)=>{loading.close();this.$msg.error(err)})
            } else {
              return false;
      ...
      receiveGoods(orderId){
        this.$confirm('请收到咖啡并检查无误后，再确认收货', '提示', {
